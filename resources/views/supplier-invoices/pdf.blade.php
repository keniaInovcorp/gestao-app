...
<body>
    <div class="header">
        ...
        <div class="document-header-right">
            <div class="client-company-name">{{ $invoice->supplier->name ?? '' }}</div>
    ...
    <div class="client-details-section">
        <div class="client-details-header">
            <div class="client-details-left">
                <div class="client-detail-item"><strong>Fornecedor Nº:</strong> {{ $invoice->supplier->number ?? '' }}</div>
                <div class="client-detail-item"><strong>Contribuinte:</strong> {{ $invoice->supplier->tax_number ?? '' }}</div>
            </div>
            <div class="client-details-right">
                <div class="client-detail-item"><strong>Edição:</strong> 1</div>
                <div class="client-detail-item"><strong>de:</strong> {{ $invoice->invoice_date ? $invoice->invoice_date->format('d M Y') : '-' }}</div>
                <div class="client-detail-item"><strong>Vencimento:</strong> {{ $invoice->due_date ? $invoice->due_date->format('d M Y') : '-' }}</div>
            </div>
        </div>
    </div>

    <div class="invoice-header">ENCOMENDA FORNECEDOR #{{ $invoice->supplierOrder->number ?? '' }}</div>
    ...
    <table class="items-table">
        ...
        <tbody>
            @if($invoice->supplierOrder && $invoice->supplierOrder->lines)
                @foreach($invoice->supplierOrder->lines as $index => $line)
                <tr>
    ...
    <div class="summary-section">
        <div class="summary-left">
            <table class="summary-table">
                <tr>
                    <td class="label"><strong>Encomenda Fornecedor:</strong></td>
                    <td class="value">{{ $invoice->supplierOrder->number ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="label"><strong>Data Encomenda:</strong></td>
                    <td class="value">{{ $invoice->supplierOrder->order_date ? $invoice->supplierOrder->order_date->format('d M Y') : '-' }}</td>
                </tr>
                <tr>
        ...
        <div class="summary-right">
            @php
                ...
                $vatPercentage = 0;
                if($invoice->supplierOrder && $invoice->supplierOrder->lines) {
                    foreach($invoice->supplierOrder->lines as $line) {
            ...
            <table class="summary-table">
                ...
                <tr>
                    <td class="label">IVA ({{ number_format($vatPercentage, 2, ',', '.') }}%):</td>
