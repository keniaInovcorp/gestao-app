...
const baseRole = computed(() => {
    if (!props.user?.roles || props.user.roles.length === 0) return null;
    return props.user.roles.find(r => r.name === 'admin' || r.name === 'regular');
});

const permissionGroupRole = computed(() => {
    if (!props.user?.roles || props.user.roles.length === 0) return null;
    return props.user.roles.find(r => r.name !== 'admin' && r.name !== 'regular');
});

const selectedRoleId = ref(baseRole.value?.id.toString() || '');
...
watch(() => props.user, () => {
    ...
    if (base) {
        selectedRoleId.value = base.id.toString();
        inertiaForm.role_id = base.id.toString();
    }
...
const onSubmit = handleSubmit((values) => {
    ...
    if (values.password && values.password.trim() !== '') {
        ...
        inertiaForm.put(`/users/${props.user.id}`);
    } else {
