<template>
    <AuthenticatedLayout>
        <Head title="Editar Ação de Calendário" />
        <div class="py-8">
        <div class="mb-6">
            <Link href="/calendar-actions" class="text-blue-600 hover:text-blue-900 mb-4 inline-block">
                ← Voltar
            </Link>
            <h1 class="text-2xl font-bold">Editar Ação de Calendário</h1>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form @submit="onSubmit">
                ...
                <div class="flex gap-4 mt-6">
                    ...
                    <Button type="button" variant="outline" @click="router.visit('/calendar-actions')">
                        Cancelar
                    </Button>
                </div>
            </form>
...
const { handleSubmit } = useForm({
    ...
    initialValues: {
        ...
        status: props.calendarAction?.status || 'active',
    },
});
...
const onSubmit = handleSubmit((values) => {
    ...
    inertiaForm.put(`/calendar-actions/${props.calendarAction.id}`);
});
</script>
